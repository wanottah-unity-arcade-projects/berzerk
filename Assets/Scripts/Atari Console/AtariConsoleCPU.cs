...
public class AtariConsoleCPU : MonoBehaviour
{
    ...
    public Transform gameCartridgeTransform;

    public Transform greyScalePostProcessing;
    ...
    [SerializeField] private Text initialisingConsoleText;
    ...
    #region ATARI CONSOLE CONTROL MODES
    ...
    // game cartridge
    public bool gameCartridgeInserted;
    ...
    private void PowerUpConsole()
    {
        ...
        CheckGameCartridgeInserted();

        // if a game has been insterted, then read the game cartridge data
        /*if (gameCartridgeInserted)
        {
            noCartridgeInsertedText.gameObject.SetActive(CONTROL_INACTIVE);
        ...
        {
            noCartridgeInsertedText.gameObject.SetActive(CONTROL_ACTIVE);
        }*/
    ...
    public void CheckGameCartridgeInserted()
    {
        if (initialisingConsoleSystem)
        {
            StartCoroutine(InitialiseConsoleDelay());
        }

        else
        {
            StartCoroutine(GameCartridgeCheckDelay());
        }
    }


    IEnumerator InitialiseConsoleDelay()
    {
    ...
    IEnumerator GameCartridgeCheckDelay()
    {
        initialisingConsoleText.gameObject.SetActive(CONTROL_ACTIVE);

        yield return new WaitForSeconds(4f);

        initialisingConsoleText.gameObject.SetActive(CONTROL_INACTIVE);

        if (gameCartridgeTransform != null)
        {
            noCartridgeInsertedText.gameObject.SetActive(CONTROL_INACTIVE);
